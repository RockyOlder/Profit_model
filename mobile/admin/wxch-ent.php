...
if($_REQUEST['act'] == 'wxconfig') 
{
	...
	if(!empty($_POST['token']))
	{
		...
		if($ret)
		{
			sys_msg('实实微分销：提示您设置成功',0,$link);
		}
		else
		{
			sys_msg('实实微分销：提示您设置失败',0,$link);
		}
	}
	else
	{
...
{	
	...
	if($_POST)
	{
		...
		$data['shishi_url'] = $_POST['shishi_url'];
	...
	else
	{
		...
		foreach($ret as $k=>$v)
		{
			...
			switch ($v['cfg_name']) 
			{
				...
				break;
				case 'shishi_url':$wxchdata[$k]['title'] = '分销引导关注URL';
				break;
