...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
	...
	/*实 实--微 分 销 -获取授-权-域名*/
	$smarty->assign('shop_url', $db->getOne('SELECT value FROM ' . $ecs->table('touch_shop_config') . ' WHERE code = "shop_url"'));
	$sv_url = $_SERVER['SERVER_NAME'];
    $smarty->assign('sv_url',  $sv_url);
