...
if ($action == 'default')
{
 //getOne
	...
	if($rank_points>$level_register_up||$rank_points==$level_register_up){
		...
		$shishi="掌柜：否     <a href='./'>点此链接购买成为掌柜</a>";
		$shishiflag="";
	}
        $smarty->assign('shishiflag',        $shishiflag);
        	$ret = $db->getAll("SELECT * FROM `wxch_qr`  WHERE `affiliate` !=0 AND `affiliate`=".$user_id." ORDER BY `qid` DESC LIMIT 0,1");
           //     print_r($ret);exit;
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
  ...
  //  include_once(ROOT_PATH .'include/lib_clips.php');
   ...
   // print_r($up_uid);exit;
        ...
        if ($count)
        {
			...
			foreach($user_info as $key=>$value){
				...
				$shishiarr['yifukuan']['setmoney']+=$setmoney;
                                ...
                                $sql = "SELECT COUNT(*)as ordercount,SUM(goods_amount)as goods_count FROM " .$GLOBALS['ecs']->table('order_info')." WHERE user_id IN($up_uid) AND add_time > '" .local_strtotime('-1 months'). "' and  shipping_status  =2";
                                
                                $orderCount =$db->getRow($sql);
                                if(!empty($orderCount)){
                                  //  print_r($orderCount);
                                    $orderCountStatistical['order_count'] =     $orderCountStatistical['order_count']+$orderCount['ordercount'];
                                    $orderCountStatistical['goods_count'] =     $orderCountStatistical['goods_count']+$orderCount['goods_count'];
                                }
                                $userCount =$db->getRow("SELECT SUM(pay_points)as user_integral FROM " .$GLOBALS['ecs']->table('account_log')." WHERE user_id IN($up_uid) AND change_time > '" .local_strtotime('-1 months'). "'");
                                $orderCountStatistical['user_count']      =   $orderCountStatistical['user_count']+$userCount['user_integral'];
                                
				//$sql="SELECT count(*) FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']."  and  shipping_status  =2";
    ...
    //print_r($orderCountStatistical);exit;
	$shishiall=array();
    ...
    $smarty->assign('orderCountStatistical',        $orderCountStatistical);
    $smarty->assign('all_count',        $all_count);
    $smarty->assign('shishiall',        $shishiall);
        //////////////////////////
    $smarty->assign('wxchdata',current($wxchdata));
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        {
			$sql="UPDATE ".$ecs->table('users'). "SET `ec_salt`='0',`password_shishi`='$new_password' WHERE user_id= '".$user_id."'";
			$db->query($sql);
            $user->logout();
...
//     print_r($pager);exit;
  //  var_dump($_GET['status']);exit;
    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$_GET['status']);
...
elseif ($action == 'async_order_list')
{
    ...
    if($_POST['status']=='null' && $_POST['shipping_status']=='null'){
            $orders = get_user_orders($user_id, $limit, $start);
    } elseif(isset ($_POST['status']) && $_POST['shipping_status']=='1') {
            $orders = get_user_orders($user_id, $limit, $start,$_POST['status'],$_POST['shipping_status']);
    }else{
            $orders = get_user_orders($user_id, $limit, $start,$_POST['status']);
    }
    ...
    if(is_array($orders)){
        foreach($orders as $vo){
            ...
            <tr>
                <td><img src="'.$config['site_url'].$img.'" width="60" height="60" style="border-radius:0.5rem;"/></td>
...
//print_r($_POST);exit;
    $address_message =  $_POST['province_name'].$_POST['city_name'].$_POST['district_name'];
    $address = array(
        'user_id'    => $user_id,
    ...
    if(!empty($address_message)){
        $address['address_message'] =   $address_message;
    }
