...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $smarty->assign('related_goods',    article_related_goods($_REQUEST['id']));  // 相关推荐商品
    $smarty->assign('id',               $article_id);
    $smarty->assign('username',         $_SESSION['user_name']);
    $smarty->assign('email',            $_SESSION['email']);
    $smarty->assign('type',            '1');
    $smarty->assign('promotion_info', get_promotion_info());
	/*------------------------bof ShiShiWFX 2015.06.4-----------------*/
	//bof ShiShiWFX 2015.06.4新增文章阅读次数调用
	$count = $db -> getOne("select click_count from  ".$ecs->table('article')." where article_id = '$article_id'");
	$praise = $db -> getOne("select love from  ".$ecs->table('article')." where article_id = '$article_id'");
	 if($count>100000)
	 {
		$count = "100000+";
	...
	//bof ShiShi WFX 2015.06.4新增文章点赞次数调用
	 $smarty->assign('praise',$praise);
	/*------------------------eof ShiShi WFX-----------------*/
/*--------------------start 增加文章详情页关注微信公众号链接调用------------*/
...
$smarty->display('article.dwt', $cache_id); //文章详细页 by shishiWFX
...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.sales_volume_base, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['sales_volume']  = $row['sales_volume_base'];//ShiShi WFX add 获取相关商品销量基数
...
/*-------------------bof ShiShi-WFX------------------------*/
/* 更新点击次数 */
$db -> query("update ".$ecs->table('article')." set click_count=click_count + 1, rel_clickcount=rel_clickcount + 1 where article_id = '$article_id'");

if(@$_REQUEST['act'] == 'chage_usenum')
{
$res = array('err_msg' => '', 'result' => '', 'qty' => 1);
$id = $_REQUEST['id'];
$sql = $GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('article')." set love = love+1 where article_id='$id'");

$res['id']= $id;
$res['usenum'] = $GLOBALS['db']->getOne("select love from ".$GLOBALS['ecs']->table('article')." where article_id='$id'");
	echo $res['usenum'];
}
//*-------------------eof Shi-Shi-WFX------------------------*/
