...
/*--------------------------实 实 微 分 销 对域名进行授权 start-----------------------------------*/
$shop_url = $GLOBALS['db']->getOne('SELECT value FROM ' . $ecs->table('touch_shop_config') . " WHERE code = 'shop_url'");
if(!in_array($_SERVER['HTTP_HOST'],array($shop_url))){exit('<h1>请购买程序正版授权：<a href="http://bbs.hanyu168.com">http://bbs.hanyu168.com</a></h1>');}
/*--------------------------对域名进行授权 end-----------------------------------*/
...
//*20150608实实微分销独家开发新增*/
	...
	//echo $phone_state.'|';
	//echo $u.'|';
	$sql_one="SELECT * FROM " . $GLOBALS['ecs']->table('ip_log') . " WHERE phone_state='$phone_state' and u_id='$u' ";	
	$ipinfo1=$GLOBALS['db']->GetRow($sql_one);
	//print_r($ipinfo1);
	if(empty($ipinfo1)&&!empty($u)){
		//将IP和推荐会员的u存入数据库中，从而获得会员上下级关系

		$sql = "INSERT INTO ".$GLOBALS['ecs']->table('ip_log')."(u_id,state,phone_state) "."VALUES ('$u','$state','$phone_state')";
		$GLOBALS['db']->query($sql);
		//记录结束		
	}
//新增绑定上下级关系开始
	//$iipp = $_SERVER["REMOTE_ADDR"];
	$sql_two="SELECT * FROM " . $GLOBALS['ecs']->table('ip_log') . " WHERE state=0 and phone_state='$phone_state' ORDER BY id DESC LIMIT 0 , 1
";	
...
/*避免上下级关系混乱，实实-微分销修复*/
					...
					$childinfo_shishi=$GLOBALS['db']->GetAll($sql);
					
					$flag_shishi=true;
					if(!empty($childinfo_shishi)){
						$flag_shishi=false;
					}else{
						$flag_shishi=true;
					}
...
/*避免上下级关系混乱，实实微分销修复*/
	...
	if($userinfo['parent_id']==0&&$flag&&$user_id&&$flag_shishi){
	

        $affiliate  = unserialize($GLOBALS['_CFG']['affiliate']);
		$up_uid = $_COOKIE['ecshop_affiliate_uid'];//2015-8-1 实--实-微-分--销-修复 分享链接发展下级会员功能
