...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_name, reg_time, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
  ...
  //  print_r($row);exit;
	...
	$info['outouser_id'] = $row['user_name'];//shishi- wfx add 获取用户自动注册网站ID
	$info['reg_time'] = local_date('Y-m-d', $row['reg_time']);//shishi  wfx add 获取用户关注时间
