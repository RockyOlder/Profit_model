...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.sales_volume_base, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['sales_count']  = get_sales_volume($row['goods_id']) + $row['sales_volume_base'];//shishi  wfx add 
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['short_style_name'] = add_style($goods[$idx]['short_name'], $row['goods_name_style']);
		$goods[$idx]['sales_count']  = get_sales_volume($row['goods_id']) + $row['sales_volume_base'];//shi-shi wfx add 
        $idx++;
    }
...
{
    /* 对属性进行重新排序和分组 */
    $sql = "SELECT attr_group, g.is_shipping ".
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
														...
														'is_shipping' => $groups[$row['is_shipping']]);//shi-shi wfx add产品详情页显示运费情况
        }
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$goods[$idx]['sales_count']  = get_sales_volume($row['goods_id']) + $row['sales_volume_base'];//shish-wfx add 
    }
