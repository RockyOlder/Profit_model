 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0,$status=null,$shopinng=null)
{
   // $status='0';
    ...
    } elseif($status!=null && $shopinng=='1') {
            $sql = "SELECT order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
                   "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
                   " FROM " .$GLOBALS['ecs']->table('order_info') .
                   " WHERE user_id = '$user_id' AND order_status='$status' AND shipping_status='$shopinng' ORDER BY add_time DESC";
    }else{
            $sql = "SELECT order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
                   "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
                   " FROM " .$GLOBALS['ecs']->table('order_info') .
                   " WHERE user_id = '$user_id' AND order_status='$status' ORDER BY add_time DESC";
    }
   // print_r($sql);exit;
