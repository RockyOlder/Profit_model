            ...
            else
            {
				...
				if($rank_points>$level_register_up||$rank_points==$level_register_up){
					...
					$uri = $rewrite ? 'article-' . $aid : 'article.php?id=' . $aid."&u=".$userid;
							...
							$uri = $rewrite ? 'article-' . $gid : 'article.php?id=' . $gid;
						}
					}else{
 ...
 * 调用get_pro_number()函数
 *
 * @param   array  $pro_number 产品编号
 *
 * @return  $final_pro_number 返回对应产品颜色尺码库存量
 */
//bof Shi-ShiWFX 新增不同颜色尺码库存数量联动功能
function get_pro_number($pro_number = array())
{
	if(!empty($pro_number))
	{
		$is_s=sort($pro_number);
		if ($is_s)
			$goods_attr = implode("|", $pro_number);
		
		$final_pro_number = $GLOBALS['db']->getOne("SELECT product_number FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_attr='".$goods_attr."'");
		return $final_pro_number;
	}
}
