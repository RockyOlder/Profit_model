...
class wechatCallbackapi {
	...
	public function responseMsg($db, $user, $base_url) {
		...
		if (!empty($postStr) or $debug == 1) {
			...
			$user_password=$db -> getOne("SELECT `password_shishi` FROM `$thistable` WHERE `wxid` ='$fromUsername'");
			//实-实    新增绑定多用户ID
...
( '$fromUsername', '$keyword', $time);");
					...
					if (empty($wxch_user_name)) {
						...
						if (empty($wxch_nobd_wxid)) {
							...
							if ($uc_state) {
								...
								if($autoreg_state){
									$wxch_user_sql = "INSERT INTO `$thistable` ( `user_name`,`password`,`wxid`,`user_rank`,`wxch_bd`,`password_shishi`,`reg_time`) VALUES ('$fromUsername','$ec_pwd','$fromUsername','99','no','$ec_pwd_no','$time')";
									$db -> query($wxch_user_sql);
									$ecs_user_id = $db -> insert_id();
									...
									$db -> query($ecs_update);
									//注册后默认绑定
									$db->query("UPDATE `wxch_user` SET `setp`= 3,`affiliate`='$ecs_user_id',`uname`='$ecs_user_name' WHERE `wxid`= '$fromUsername';");//shish- wfx Q1141881310 add `affiliate`赋值
			...
			if ($postObj -> Event == 'subscribe') {
				if (strlen($postObj -> EventKey) == 0) {
					...
					$shishi_users = $db -> getRow("SELECT * FROM `ecs_users` WHERE user_id =$scene_id");
					
					if(!empty($shishi_users)){
			...
			if ($setp == 10) {
				...
				$password = $keyword;
				//实实--微分销修改兼容点击菜单直接解除绑定BUG
				if(empty($password)){
					$password= $postObj -> EventKey;
				}
				//实实微分销修改兼容点击菜单直接解除绑定BUG				
...
( '$fromUsername', '$menu_message', $time);");
			...
			if ($keyword == 'debug') {
				...
				$imgs = $name;
				$target = '../qrcode/shishiwfx.jpg';//背景图片
			...
			if ($aff_arr[0] == 'affiliate') {
				if (!empty($aff_arr[2])) {
					...
					if(empty($parent_id)){
						if (!empty($aff_db)&&$flag&&$flageshishi) {
							//绑定会员账号
							...
							$msgType = "text";
							$contentStr=$nick_name."恭喜您由".$parent_name."推荐成为".$shop_name."的会员！目前已经有【".$num_user."】位掌柜！点击左下角“".$shop_name."”立即加入成为".$shop_name."掌柜，裂变你的代理商，让你每天睡觉都能赚大钱！";
							$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
							$this -> insert_wmessage($db, $fromUsername, $contentStr, $time, $belong);
							$this -> universal($fromUsername, $base_url);
							echo $resultStr;
			...
			/*实--实-微-分-销   修复   新增找回密码功能 关键字getback*/
				$password_shishi =  $GLOBALS['db']->getOne("SELECT password_shishi FROM " . $GLOBALS['ecs']->table('users')."WHERE `wxid` = '$fromUsername'");
				$user_name =  $GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['ecs']->table('users')."WHERE `wxid` = '$fromUsername'");
				$msgType = "text";
				$contentStr = "您的账号是：".$user_name ."\n您的密码是：".$password_shishi;
					$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
					$this -> insert_wmessage($db, $fromUsername, $contentStr, $time, $belong);
					$this -> universal($fromUsername, $base_url);
					echo $resultStr;
		...
		if($ticket)
		{
			...
			if(false !==$local_file){
				...
				/*实-实--微-分销修复  换域名问题和头像重新生成图片问题*/
				$scene=$user_name;				
				if(!empty($qr_path)){
				/*实- 实- 微 -分-销修复  换域名问题和头像重新生成图片问题*/
					 $surl=$qr_path;
		...
		if($ticket)
		{
			...
			if(false !==$local_file){
				...
				imagecopy($target_img,$source,165,430,0,0,200,200);
				...
				echo $resultStr;
				exit;
			}elseif($keyword == 'tianxin') {
			
				$qr_path = $db->getOne("SELECT `qr_path` FROM `wxch_qr_tianxin100` WHERE `scene_id`='$scene_id'");
				if(!empty($qr_path)){			
					$sql = "DELETE FROM `wxch_qr_tianxin100` WHERE  `scene_id`='$scene_id'";
					$db->query($sql);	
				}
				...
				/*实-实：微-分-销修复  换域名问题和头像重新生成图片问题*/
				$scene=$user_name;
				$nickname_arr = $db->getAll("SELECT `nickname` FROM `wxch_qr_tianxin100` WHERE `scene_id`='$scene_id' order by id desc");				
				$nickname_new=$nickname_arr[0];
				$nickname = $db->getOne("SELECT `nickname` FROM `wxch_user` WHERE `wxid`='$fromUsername'");
				$tianxin100_flag=false;
				/**/
				if($nickname_new == $nickname){
					$tianxin100_flag=true;
				}
				
				if(!empty($qr_path)&&$tianxin100_flag){
				/*实---实--微-分-销 修复  换域名问题和头像重新生成图片问题*/
					 $surl=$qr_path;
					 $data=dirname(__FILE__)."\qrcode\/".$surl;
					 }else{
				$action_name="QR_LIMIT_SCENE";
...
nation=大雁塔&mode=driving&region=西安';
             ...
             <Url><![CDATA[" . $url . "]]></Url>
				...
				/* 实---实-微--分-销  新 增修 复  如果是直接关注的则默认分配上级账号避免推荐关系混乱*/
				$affiliate = unserialize($GLOBALS['_CFG_MOBILE']['affiliate']);
